...
def read_file(file_name):
    with open(os.path.join("inaugural", file_name), "rb") as file:  # Open in binary mode
        rawdata = file.read()
        result = chardet.detect(rawdata)
        encoding = result["encoding"]
        # print(encoding)

    with open(os.path.join("inaugural", file_name), "r", encoding=encoding) as file:
        file_text = file.read()
    return file_text
...
def get_president_sentences(president):
    files = sorted([file for file in os.listdir("inaugural") if president.lower() in file.lower()])
